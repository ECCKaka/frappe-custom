...
class BaseDocument(object):
	...
	def get(self, key=None, filters=None, limit=None, default=None):
		# print('base document 109', key, filters, limit, default)
		if key:
			if isinstance(key, dict):
				print('base documents is instance of dict 112', key)
	...
	def get_valid_dict(self, sanitize=True, convert_dates_to_str=False, ignore_nulls = False):
		d = frappe._dict()
		# print('self.meta.get_valid_columns() 209', self.meta.get_valid_columns())
		# print("frappe.local.valid_columns 210  ", frappe.local.valid_columns['Custom Floorplan'])
		for fieldname in self.meta.get_valid_columns():
			# print('fieldName 212', fieldname)
	...
	def get_valid_columns(self):
		if self.doctype not in frappe.local.valid_columns:
			...
			# if self.doctype == "Custom Floorplan":
			# 	print('base document 271', valid)
	...
	def db_update(self):
		if self.get("__islocal") or not self.name:
			print('self.db_insert() 348')
			self.db_insert()
			return

		d = self.get_valid_dict(convert_dates_to_str=True, ignore_nulls = self.doctype in ('DocType', 'DocField', 'DocPerm'))
		# print('db_update() 353  ', d)
		...
		# ['owner', 'creation', 'modified', 'modified_by', 'parent', 
		# 'parentfield', 'parenttype', 'idx', 'docstatus', 'attachimage', 'geolocation', 
		# 'amended_from', 'frontech_custom_geolocation']
		print('after columns ',", ".join(["`"+c+"`=%s" for c in columns]))
		try:
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (frappe.flags.in_test and value.startswith("_T-")):
				...
				print('base document 558 ', df.fieldname, options)
